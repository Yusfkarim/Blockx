...
class SettingsScreenDetector @Inject constructor(@param:ApplicationContext private val context: Context) {
    ...
    /** The core Settings hosts whose whole pipeline gets the whitelist-first treatment. */
    private fun isCoreSettingsHost(packageName: String): Boolean =
        packageName == "com.android.settings" || packageName == "com.samsung.android.settings"

    /**
     * Strict allowlist for core Settings hosts: only app-management/uninstall, accessibility
     * service management, device-admin or force-stop surfaces may arm the guard there — general
     * categories never do. Screens showing OUR identity with a destructive action stay guarded.
     */
    private fun isSensitiveCoreSettingsSurface(activityClass: String, scan: ScanResult): Boolean {
        if (activityClass.isNotBlank()) {
            val lowered = activityClass.lowercase()
            if (CORE_SETTINGS_SENSITIVE_CLASS_TOKENS.any { lowered.contains(it) }) return true
        }
        if (scan.matchesOwnApp && scan.destructiveOwnApp) return true
        return false
    }

    /** Sensitive activity-class tokens inside the core Settings hosts. */
    private val CORE_SETTINGS_SENSITIVE_CLASS_TOKENS = listOf(
        // App management / uninstall routes
        "applicationsstate", "installedappdetails", "appdetails", "appheader",
        "manageapplications", "installedappslist", "appmanager", "appmanagementactivity",
        "apppermission", "managepermissions", "permissionapps",
        // Accessibility service management
        "accessibilitysettings", "accessibilitydetailssettings", "toggleaccessibilityservice",
        "accessibilityserviceconfirmation", "installedaccessibility", "accessibilityservice",
        // Device admin & force-stop / uninstall dialogs
        "deviceadminsettings", "deviceadminadd", "devicepolicy", "managedeviceadmin",
        "deviceadminreceiver", "deviceadministrator",
        "uninstalleractivity", "uninstallappactivity", "uninstallalert", "uninstallconfirm",
        "factoryreset", "masterclear",
    )
    ...
    fun detect(packageName: String, className: String?, root: AccessibilityNodeInfo?): ProtectedScreenDetection? {
        ...
        // Whitelist-first for the core settings hosts (Samsung One UI / Galaxy M32 5G field
        // report): with the whitelist active, ONLY explicit sensitive routes (app management /
        // uninstall / accessibility-service management / device-admin or force-stop dialogs) may
        // challenge; base navigation shells (SecSettings2Activity / SecMainSettingsActivity /
        // SubSettings) and general categories (Wi-Fi, Sound, Display, Device Care, Accounts)
        // are ignored immediately without any further pipeline work.
        if (isCoreSettingsHost(packageName) && !isSensitiveCoreSettingsSurface(activityClass, scan)) {
            return null
        }
