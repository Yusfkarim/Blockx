...
class ShieldAccessibilityService : AccessibilityService() {
    ...
    override fun onServiceConnected() {
        ...
        serviceInfo = serviceInfo.apply {
            eventTypes = AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED or
                ...
                // Long-press is an interception path of its own (e.g. long-press "Power off" to
                // reveal safe mode, long-press a wipe/reset row). The runtime eventTypes
                // bitmask overrides the XML typeAllMask, so without this entry those events
                // never reached the click-time settings-protection detectors.
                AccessibilityEvent.TYPE_VIEW_LONG_CLICKED or
                AccessibilityEvent.TYPE_VIEW_SELECTED or
                AccessibilityEvent.TYPE_VIEW_FOCUSED or
                AccessibilityEvent.TYPE_VIEW_SCROLLED or
                // Compose and WebView based apps (ChatGPT, Gemini, many chat clients) report
                // edits only through content changes, never through TYPE_VIEW_TEXT_CHANGED.
                AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED or
                // Submit paths: Enter, IME search action and cursor traversal after voice
                // input or an autocomplete selection.
                AccessibilityEvent.TYPE_VIEW_TEXT_TRAVERSED_AT_MOVEMENT_GRANULARITY or
                AccessibilityEvent.TYPE_VIEW_ACCESSIBILITY_FOCUSED
            feedbackType = AccessibilityServiceInfo.FEEDBACK_GENERIC
            // Must be 0. A non-zero timeout makes the framework coalesce events and hold the
            // last one of a burst until some unrelated event flushes the queue, which is why
            // the first typed keyword was only caught after opening the notification shade.
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        // display, every heavy layout re-scan or window-ejection we run would freeze/collapse
        // the user's keyboard mid-word. Deterministic check: an active TYPE_INPUT_METHOD
        // window OR a currently-focused editable node. Text-analysis branches (typing-based
        // keyword/URL intelligence) are evaluated below regardless — they only read text.
        val imeVisible = isImeVisibleNow(event)
        ...
        if (shortVideoPlatform != null && shortVideoPreferences.isEnabled(shortVideoPlatform)) {
            ...
            // IME isolation: scrolling/typing inside a Reels comment thread must never trigger
            // the heavy window scan — it would collapse the soft keyboard mid-word.
            val willScanWindow = wantsWindowScan && scanAllowed && !imeVisible
        ...
        if (TelegramSearchPolicy.isTelegram(packageName) && telegramSearchPreferences.isEnabled()) {
            ...
            if (!TelegramSearchPolicy.isChatContext(lastTelegramActivity)) {
                ...
                if (!interactionHit && type == AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED && !imeVisible) {
                    val now = System.currentTimeMillis()
    ...
    /**
     * IME/typing-session detector: true while a soft keyboard is on screen (any window of
     * TYPE_INPUT_METHOD visible, or an editable node has input focus). Used to pause reset &amp;
    ...
    private fun guardAppSettings(
        ...
        // The IME/keyboard state must be evaluated where the dispatch lives — the challenge
        // handler is invoked from both onAccessibilityEvent and the debounce runnable, and only
        // the live event can tell us whether typing is in flight.
        val imeVisible = isImeVisibleNow(event)
        ...
        when (decision) {
            SettingsProtectionGuard.Decision.CHALLENGE -> {
                // Typing-surface priority (field fix): if the soft keyboard currently owns the
                // frame (the user is drafting a message inside Settings search, Telegram global
                // search, a browser omnibox…), a PIN challenge would swallow the IME mid-word.
                // Defer to the regular recheck cadence — the very next settled frame catches it.
                if (imeVisible) {
                    Log.d(TAG, "challenge deferred while IME visible: pkg=$packageName class=$className")
                    scheduleSettingsProtectionRecheck(packageName, className)
                    return
                }
                // Samsung One UI absolute safeguard (Galaxy M32 5G / Android 13): the base
                // navigation hosts (SubSettings, SecSettings2Activity, SecMainSettingsActivity)
                // can never be HOME-ejected — only the explicitly sensitive classes identified
                // by the detector upstream survive to a challenge. Verified+logged here.
                val loweredClass = className?.lowercase().orEmpty()
                if (looksLikeSettingsPackage(packageName) &&
                    ONE_UI_BASE_SETTINGS_HOSTS.any { loweredClass.contains(it) }
                ) {
                    Log.d(
                        TAG,
                        "bypass Samsung base container: pkg=$packageName class=$className",
                    )
                    return
                }
                Log.d(TAG, "challenge pkg=$packageName class=$className")
    ...
    companion object {
        ...
        private const val TAG = "ShieldGuard"

        /** One UI base navigation containers — never home-ejectable, even mid-race. */
        private val ONE_UI_BASE_SETTINGS_HOSTS = listOf(
